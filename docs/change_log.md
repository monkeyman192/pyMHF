...
- Added `pymhf.core.hooking.get_caller(log: bool)` decorator for detours. When added, this will determine the location the function was called from. To see this value automatically pass `log=True` in to the decorator. ([gh-34](https://github.com/monkeyman192/pyMHF/issues/34)). NOTE: This currently will only work for 64bit applications.
